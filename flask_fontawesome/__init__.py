...
class StaticCdn(Cdn):
    ...
    def __css(self, style: str, use_min: bool) -> Markup:
        min_str = ".min" if use_min else ""
        resource = "css/{}{}.css".format(style, min_str)
        return Markup('<link href="{url}" rel="stylesheet">').format(url=self.__url(resource))

    def __js(self, style: str, use_min: bool) -> Markup:
        min_str = ".min" if use_min else ""
        resource = "js/{}{}.js".format(style, min_str)
        return Markup('<script defer src="{url}" rel="stylesheet"></script>').format(
...
class UseFontAwesomeComCdn(Cdn):
    ...
    def __css(self, style: str, integrity: str) -> Markup:
        url = "{}/css/{}.css".format(self.__URL_BASE, style)
        return Markup(
            '<link rel="stylesheet" href="{url}" integrity="{hash}" crossorigin="anonymous">'
        ).format(url=url, hash=integrity)

    def __js(self, style: str, integrity: str) -> Markup:
